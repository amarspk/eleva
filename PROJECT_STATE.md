...
Post-Step-4 approved fixes (one isolated commit each): **R4 `c15843f`**, **R5 `084dd37`** (both 2026-07-30 — details §5 table and §19). Remaining verified runtime defects: **R1 = LATENT-KDS-001 (CONFIRMED), R2 = PREX-MIG-002, R3 = PREX-MIG-003** — all OPEN, awaiting fix-scope decisions (§20).
...
| LATENT-KDS-001 (R1) | `kds.service.ts:76` `orderBy: { createdAt: 'asc' }` on `KitchenQueue`, a model with **no `createdAt` column** (schema verified) → Prisma runtime `ArgumentValidationError` on KDS REST ticket list (initially CANNOT CONFIRM without runtime; since confirmed — see Status) | OPEN — **CONFIRMED 2026-07-30 (Step-4 live run):** `GET /api/v1/kds/tickets?branchId=…` → HTTP 500; API log `Unknown argument 'createdAt'. Available options are marked with ?.` Fix scope deferred (user decision pending) | kds |
...
2. **Open runtime-defect fix decisions (awaiting user scoping):** R1 = LATENT-KDS-001 (`kds.service.ts:76`, CONFIRMED 2026-07-30), R2 = PREX-MIG-002 and R3 = PREX-MIG-003 (broken migrations 4/5, CONFIRMED 2026-07-30). Not started; no fixes without approval.
...
5. Build-gate remediation (PREX-TSC-001/002): 145 api + 16 db legacy type errors breaking `pnpm build`/CI Job 3/Docker builder.
...
|---|---|---|---|---|
| PREX-TSC-001 | 145 api tsc errors; `tsc` is the api build script | OPEN | Critical | Sprint 1 Step 5 (`turbo build` criterion); also CI Job 3 and the Docker builder |
...
| LATENT-KDS-001 (R1) | `kds.service.ts:76` orderBy nonexistent `createdAt` → `GET /api/v1/kds/tickets` HTTP 500 | OPEN — CONFIRMED 2026-07-30 | High | KDS REST consumers; the next KDS-scope work (outside the Sprint-1 gate) |
...
- **Major Defects: 5** — PREX-TSC-002, PREX-E2E-001, PREX-MIG-002 (R2), PREX-MIG-003 (R3), ENV-TSK-0006. (Plus 1 High-severity confirmed runtime defect, LATENT-KDS-001/R1 — outside the Sprint-1 gate scope.)
- **Minor Defects: 4** — LATENT-SUB-001, LATENT-MEDIA-001 (compile-time only), LATENT-ENV-001, SPEC-DRIFT.
- **Can Current Step Be Closed? NO.**
- **Reason (verified evidence only):** (1) CTO Rule 5 — a step cannot close before its verification has run, and Step 5 has not been executed; (2) the `turbo build` criterion fails on main today (PREX-TSC-001: api `tsc` exits 2 with 145 errors; PREX-TSC-002: db `tsc` 16 errors — both re-verified 2026-07-30); (3) the "suites green" criterion currently contains 1 red cell (PREX-E2E-001) unless the CTO explicitly accepts it as documented baseline.
...
- **Type errors:** api **145** (per-module breakdown in §7 PREX-TSC-001; 149 pre-Sprint, Sprint 1 added 0, removed 4) + db **16** (BaseTenantRepository delegate types vs generated client emissions). Together they break the production build chain (`pnpm build`, CI Job 3, Docker builder).
...
2. [user] Fix-scope decisions: LATENT-KDS-001 (R1), PREX-MIG-002 (R2), PREX-MIG-003 (R3) — same identifier-disease family as R4 (fixed); no drive-by fixes allowed.
...
6. [user] Build-gate remediation ownership (145 + 16 legacy tsc errors).
7. [user] Canonical local API port decision (LATENT-ENV-001): 8000 vs 3001.
8. [user] **Push** local commits `d1c6035` … `084dd37` to `origin` (user-owned; the session has pushed nothing, ever).
