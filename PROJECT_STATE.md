...
Post-Step-4 approved fixes (one isolated commit each): **R4 `c15843f`**, **R5 `084dd37`**, **R1 (see §19 row 7)**, **R3 (see §19 row 8)**, **R6 — guest-checkout gating parity (see §19 row 9)** (all 2026-07-30 — details §5 table and §19). Guest-checkout subscription-gating parity FIXED+VERIFIED 2026-07-30 (R6, §19 row 9). Remaining verified runtime defects: **R2 = PREX-MIG-002** — CLOSED 2026-07-30 as investigation-complete (CTO decision): layer-1 partition-key identifiers proven correct, layer-2 architectural incompatibility proven (R2.1 option analysis A–F) — M4 parked non-shippable pending architecture RFC (§20 row).
...
2. **Runtime-defect ledger (2026-07-30):** R4 FIXED (§19 r5), R5 FIXED (r6), R1 FIXED (r7), R3 FIXED (r8 — standard chain M1→M2→M3→M5→M6 verified green) — identifier-disease family closed; R2 CLOSED as investigation-complete (CTO; M4 parked, R2.1 recommends F indexes-only / D HASH(id) fallback). Remaining open runtime decisions: **paymentMethod persistence only** (§15 item 3) — guest-checkout gating parity FIXED 2026-07-30 as Runtime Defect R6 (§19 row 9).
