...
export function ExecutiveOffice(): React.ReactElement {
  ...
  const invokeMutation = useMutation({
    mutationFn: async () => {
      ...
      if (tool === 'verify_implementation_file' || tool === 'analyze_implementation_file' || tool === 'apply_approved_implementation' || tool === 'apply_approved_product_implementation') {
        args.filename = noteName;
      }
    ...
    <section aria-labelledby="executive-office-heading" className="space-y-4">
      <div className="rounded-xl border border-slate-800 bg-slate-950 p-6 text-white">
        ...
        <p className="mt-1 text-sm text-slate-300">
          Platform-owner Agent console. SAFE diagnostics run immediately. Plans wait for approval. After approval, write_agent_note may write markdown under docs/agent-workspace/, write_implementation_file may write TypeScript drafts under apps/api/src/agent/implementation/, verify/analyze inspect those drafts, apply_approved_implementation may copy one verified draft to apps/api/src/agent/promoted.ts only, and apply_approved_product_implementation may copy one verified draft to packages/receipts/src/promoted-implementation.ts only. apply_patch, deploy, migrations, and secrets stay blocked.
        </p>
      </div>
