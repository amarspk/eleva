...
const docs: ControllerDoc[] = [
  ...
  },
  {
    ...
    endpoints: [
      ...
      { method: 'chat', summary: 'Natural-language Agent turn (Slice 3). LLM cannot execute tools; orchestrator allowlists SAFE tools and records proposals only', auth: 'bearer', tenant: 'platform-global', permission: 'PLATFORM_OWNER (create:Agent)', body: 'ChatAgentRequest', response: 'AgentChatResponse', params: [p('sessionId', 'Agent session UUID.')], errors: [400, 401, 403, 404] },
      { method: 'approveAction', summary: 'Approve a proposed Agent action and run the Slice 4 controlled verification layer (destructive tools stay blocked)', auth: 'bearer', tenant: 'platform-global', permission: 'PLATFORM_OWNER (update:Agent)', body: 'DecideAgentActionRequest', response: 'AgentDecisionResponse', params: [p('sessionId', 'Agent session UUID.'), p('actionId', 'Agent action UUID.')], errors: [400, 401, 403, 404, 409] },
